(function() {
    ...
    var queue = []; // gradle targets are queued up until we load outline.
    ...
    function loadOutline() {
        ...
        if (!loading) {
            ...
            var u = new Ajax.Updater(document.getElementById("side-panel"),
                rootURL+"/descriptor/hudson.plugins.gradle.GradleTargetNote/outline",
    ...
    Behaviour.register({
        // insert <a name="..."> for each Gradle target and put it into the outline
        "b.gradle-target" : function(e) {
            handle(e);
        }
    });
}());

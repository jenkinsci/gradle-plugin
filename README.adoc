...
> **Note**
> - Build scans published on the public instance (https://gradle.com) are not eligible.
...
It is also possible to enable injection for specific repositories by specifying VCS repository patterns. These are comma-separated values, which will be used in a contains check against the repository URL.
For example, by specifying the filter value as `foo, bar, baz`, the injection will be performed for all Git VCS repositories which contain at least one of specified values in their VCS URL.

image::general-git-vcs-repository-filtering.png[Git VCS repository filtering, {thumbnail}]

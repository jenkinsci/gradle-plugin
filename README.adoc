...
[IMPORTANT]
=====
Starting from plugin version `2.13`, Maven extensions are not coming as bundled and require an explicit version configuration to be injected.
When upgrading from a lower version, the injection will be turned off until the version is configured.
=====

...
For example, by specifying the filter value as `foo, bar, baz`, the injection will be performed for all Git VCS repositories which contain specified values in their VCS URL.

image::general-git-vcs-repository-filtering.png[Git VCS repository filtering, {thumbnail}]

> **Note**
> - This feature is currently in Beta and requires Git Plugin to be installed. If Git Plugin is not installed, the following warning message will be show instead.

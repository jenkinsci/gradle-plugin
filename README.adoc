...
The Develocity server URL is required.
Build scan data is only fetched from this server, and never from the server named in a build scan URL found in a build log.
...
> **Note**
> - These upgrade notes only apply if you enabled the enriched build scans feature.

The Develocity server URL is now required, and is the only server the plugin fetches build scan data from.
Previously, when the field was left blank, the plugin derived the server from the build scan URL found in the build log.

If you left the field blank, enriched build scans stop working until you set the URL in the Jenkins global system configuration.
If you publish build scans to more than one Develocity instance, enriched data is now only available for the instance configured there.

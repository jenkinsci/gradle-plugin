...
Sometimes a Jenkins job can produce multiple build scans (for example when you are having multiple stages with gradle builds). 
To make each of these scans clearly identifiable, it is possible to add a label to each build scan.

Examples:
[source]
----
node {
  withGradle(buildScanLabel: 'Build') {
    sh './gradlew build'
  }
}
----
...
findBuildScans(buildScanLabel: 'Stage: Test')
----

This label will be displayed after the build scan url:

image::jenkins-build-scan-labeled.png[Build Scan labeled,align="center"]

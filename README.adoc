...
> **Note**
> - This feature is currently in Beta and requires Git Plugin to be installed. If Git Plugin is not installed, the following warning message will be show instead. The feature supports Gradle Pipeline/FreeStyle and Maven Pipeline job configurations (Maven FreeStyle job configurations are not supported)
...
|Next version                   | 1.17                                               | 1.11.1                                                    | 2022.3
|2.4                            | 1.16.6                                             | 1.11.1                                                    | 2022.3
|2.3.2                          | 1.16.5                                             | 1.11.1                                                    | 2022.3
|2.3.1                          | 1.16.5                                             | 1.11.1                                                    | 2022.3
...
> **Note**
> - These upgrade notes only apply if you are upgrading from the version `2.4` and were using the VCS repository filtering beta feature.

The VCS repository filtering was updated to support not only repository patterns that require the auto-injection functionality but also the ones that do not require the auto-injection.
Therefore, a new configuration format needs to be used.
In the new format repository patterns that require the auto-injection have to have the `+:` prefix.
The delimiter has to be changed from `,` to a `new line`.
For example, the existing configuration `foo, bar, baz` has to be written as follows:
```
+:foo
+:bar
+:baz
...
> **Warning**
> - The existing configuration will not be changed to the new format automatically. Please back up the existing configuration and change the format manually. After the plugin update, the old configuration will be lost.

...
public interface MavenInjectionAware {
    ...
    String JENKINSGRADLEPLUGIN_MAVEN_PLUGIN_CONFIG_EXT_CLASSPATH =
    ...
    String JENKINSGRADLEPLUGIN_MAVEN_PLUGIN_CONFIG_ALLOW_UNTRUSTED_SERVER =
    ...
    SystemProperty.Key BUILD_SCAN_UPLOAD_IN_BACKGROUND_PROPERTY_KEY =
            SystemProperty.Key.required("gradle.scan.uploadInBackground");
    SystemProperty.Key DEVELOCITY_UPLOAD_IN_BACKGROUND_PROPERTY_KEY =
            SystemProperty.Key.required("develocity.scan.uploadInBackground");
    SystemProperty.Key MAVEN_EXT_CLASS_PATH_PROPERTY_KEY = SystemProperty.Key.required("maven.ext.class.path");
    SystemProperty.Key GRADLE_ENTERPRISE_ALLOW_UNTRUSTED_SERVER_PROPERTY_KEY =
            SystemProperty.Key.optional("gradle.enterprise.allowUntrustedServer");
    SystemProperty.Key DEVELOCITY_ALLOW_UNTRUSTED_SERVER_PROPERTY_KEY =
            SystemProperty.Key.optional("develocity.allowUntrustedServer");
    SystemProperty.Key GRADLE_ENTERPRISE_CAPTURE_GOAL_INPUT_FILES_PROPERTY_KEY =
            SystemProperty.Key.optional("gradle.scan.captureGoalInputFiles");
    SystemProperty.Key DEVELOCITY_CAPTURE_FILE_FINGERPRINTS_PROPERTY_KEY =
            SystemProperty.Key.optional("develocity.scan.captureFileFingerprints");
    ...
    default boolean isInjectionEnabledForNode(InjectionConfig config, Node node) {
        ...
        Set<String> disabledNodes = CollectionUtil.safeStream(config.getMavenInjectionDisabledNodes())
                .map(NodeLabelItem::getLabel)
                .filter(Objects::nonNull)
                .collect(Collectors.toSet());

        Set<String> enabledNodes = CollectionUtil.safeStream(config.getMavenInjectionEnabledNodes())
                .map(NodeLabelItem::getLabel)
                .filter(Objects::nonNull)
                .collect(Collectors.toSet());
